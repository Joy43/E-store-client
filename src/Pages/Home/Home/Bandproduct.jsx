...
const BandProduct = () => {
  ...
  const ProductCard = ({ image, brand, productName, price, originalPrice }) => (
    <div
      className="w-72 bg-white shadow-md rounded-xl duration-500 hover:scale-105 hover:shadow-xl"
      data-aos="fade-up"
    >
      <Link to="/product">
        <img src={image} alt={productName} className="h-80 w-72 object-cover rounded-t-xl" />
        <div className="px-4 py-3">
          <span className="text-gray-400 mr-3 uppercase text-xs">{brand}</span>
          <p className="text-lg font-bold text-black truncate capitalize">{productName}</p>
          <div className="flex items-center mt-3">
            <p className="text-lg font-semibold text-black">${price}</p>
            {originalPrice && (
              <del className="ml-2 text-sm text-gray-600">${originalPrice}</del>
            )}
  ...
  return (
    <div>
      <div className="text-center p-10">
        <h1 className="font-bold text-4xl mb-4">The Band Product Here</h1>
        <h2 className="text-3xl">Touch Here</h2>
      </div>
      <ProductGrid />
    </div>
  );
